...
#### ```c printf() is missing here because <stdio.h> only declare printf() but the actual implementation of printf is stored in library files and these library files are not added yet at the assembly stage.```
